...
%     delay_ind = ceil(delay_set/soundv*fs - Noffset);
    ...
    parfor (Nline = 1:FOV,4)
        ...
        TXend = min(Nelements,Nelements - ceil((Nline - FOV/2)/beamspace));

        RXstart = max(1, Nelements/2 + 2 - ceil(Nline/beamspace));
        RXend = min(Nelements, ceil((FOV - Nline)/beamspace) + Nelements/2);

        TXbeam = beamspace*((TXstart:TXend) - 1) + ibeam;
        RXbeam = beamspace*((RXstart:RXend) - 1) + ibeam;
        realTXelement = max(1, ceil(Nline-FOV/2)/beamspace) + 1 + ...
        ...
        f_num_mask = double(abs(repmat(z,[1, length(RXbeam), length(TXbeam)])./f_num_D) > f_num);
        f_num_mask = f_num_mask./sum(f_num_mask,3); % size = (Nsample, len(RXbeam), len(TXbeam))
        f_num_mask_rx = reshape(double(abs(repmat(z,[1, length(TXbeam)])./repmat(x_ref(TXstart:TXend), [Nsample, 1])/2) > f_num), [Nsample,1,length(TXbeam)]); % size = (Nsample,Nelements)
        f_num_mask_rx = repmat(f_num_mask_rx./sum(f_num_mask_rx, 2), [1,length(RXbeam),1]);
        ...
        % delay_channel_data: Nsample+1, 2*Nelements, Nelements
        % RXbeam: 

 
%         tic
%         delay1 = sub2ind(size(delay_channel_data), ...
%                         delay, ...
%                         repmat(reshape(realRXelement, [1,length(realRXelement),1]), [Nsample,1,length(TXbeam)]), ...
%                         repmat(reshape(realTXelement, [1,1,length(realTXelement)]), [Nsample, length(RXbeam), 1])); % size = [Nsample, len(RXbeam), len(TXbeam)]
%         RFdata(:,Nline) = sum(f_num_mask_rx.*f_num_mask.*delay_channel_data(delay1),[2,3]);
%         toc
        ...
        tmp = sum(f_num_mask_rx.*f_num_mask.*delay_channel_data(delay),[2,3]);
        toc
    end
