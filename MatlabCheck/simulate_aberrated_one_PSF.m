...
% --------- parameters ---------
Npt         = 1;                    % number of simulated PSFs for one set
...
positions(1,3) = 1/1e3;                 % first point location: 1 mm
positions(2,3) = (10 + 30*rand(1))/1e3;  % adjacent point interval: 9 mm
positions(3,3) = 45/1e3;
...
[~, z_pt_start_ind] = min(abs(z - (positions(2,3) - psfsegz*lambda) )); % start index of a PSF region
[~, z_pt_end_ind] = min(abs(z - (positions(2,3) + psfsegz*lambda) ));   % end index of a PSF region
...
index3D = index3D(z_pt_start_ind:z_pt_end_ind, :,:);
% ------ Apply delay profile ------
...
%         save(fullfile(savepath,['Data_', num2str(1), '_delay_', num2str(k) '.mat']), 'psf_rf', 'psf_bb', 'speckle_rf', 'speckle_bb', 'dx', 'dz', 'depth', 'f0', 'k', 'bw', 'delay_curve');
%         saveas(fig, fullfile(savepath,['Data_', num2str(1), '_delay_', num2str(k), '.png']));
 break
end
