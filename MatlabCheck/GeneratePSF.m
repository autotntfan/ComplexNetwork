...
function GeneratePSF(Nmatlab, Npsfset)
...
% for example:
% GeneratePSF(5, 20) -> the 5-th matlab window is generating 20 PSF sets. i.e.
% the 81-th to 100-th PSF set.
tic
addpath('./Field2');
savepath = './simulation_data'; % where to save PSFs
if ~exist(savepath, 'dir')
    mkdir(savepath) % create dir
end
% --------- parameters ---------
Nptperset   = 5;                 % number of simulated PSFs for one set
% total # of psf = Npsf*Npt*4
rngseed     = (Nmatlab-1)*Npsfset + (7414:7414+Npsfset-1);  % random number generator seed
ptloc       = 10/1e3;            % point source location, i.e. the first point locates 10 mm
...
%         speckle_bb = conv2(speckle_rf.*exp(-1j*2*pi*f0*(2*newz'/soundv)), lpf, 'same');
        ...
        ylabel('Depth (mm)')
        axis image;
        save(fullfile(savepath,['Data_', num2str(Nptperset*(ii-1)+(ipt-1)), '_delay_', num2str(k) '.mat']), 'psf_rf', 'psf_bb', 'speckle_rf', 'speckle_bb', 'dx', 'dz', 'depth', 'f0', 'k', 'bw', 'delay_curve');
        saveas(fig, fullfile(savepath,['Data_', num2str(Nptperset*(ii-1)+(ipt-1)), '_delay_', num2str(k), '.png']));
