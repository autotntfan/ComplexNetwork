...
% for k = 1:4
    ...
    for Nline = 390:400
        ...
        Element = max(1, ceil((Nline-FOV/2)/beamspace)) + (iElement - iElement(1));
        ibeam = beamspace*(iElement) + 1 - ibeam;

        f_num_D = 2*(reshape(repmat(x_bf(ibeam)', [1, length(ibeam)]), [1,  length(ibeam), length(ibeam)]) - ...
                     reshape(repelem(x_ref(iElement), length(ibeam)), [1, length(ibeam), length(ibeam)])); % size = (1, len(RXbeam), len(TXbeam))
        f_num_mask_rx = double(abs(repmat(z,[1, length(ibeam), length(ibeam)])./f_num_D) > f_num);
        f_num_mask_rx = f_num_mask_rx./sum(f_num_mask_rx,2); % size = (Nsample, len(RXbeam), len(TXbeam))
        
        f_num_mask_tx = double(abs(repmat(z,[1, length(ibeam)])./repmat(x_ref(iElement), [Nsample, 1])/2) > f_num); % size = (Nsample,Nelements)
        f_num_mask_tx = repmat(reshape(f_num_mask_tx./sum(f_num_mask_tx, 2), [Nsample,1,length(ibeam)]), [1, length(ibeam), 1]);

        delay = reshape(repelem(delay_set(:,ibeam), 1,length(ibeam)), [Nsample, length(ibeam), length(ibeam)]) + ...
                repmat(delay_set(:,ibeam), [1, 1, length(ibeam)]); % size = [Nsample, len(RXbeam), len(TXbeam)]
        delay = ceil(delay/soundv*fs - Noffset);
        delay(delay>Nsample) = Nsample + 1;

 
        delay = delay + ...
                repmat((Element-1)*(Nsample+1), [Nsample,1,length(ibeam)]) + ... % 2D index
                reshape(repelem(prod(size(delay_channel_data,1,2))*(Element-1), Nsample*length(Element)), [Nsample,length(ibeam),length(ibeam)]); % 3D index
