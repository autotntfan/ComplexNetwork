function GeneratePSF_DiffChannelData(Nmatlab, Npsfset)
...
% For each set, PSFs are generated from different channel data and suffers
...
% 
% for example: Generate 200 sets (total 200*10 = 2000 PSFs) per pool.
% using_pool = 10;
% total_PSF = 2000;
% parfor ii = 1:using_pool
%   GeneratePSF_SameChannelData(ii,total_PSF/using_pool);
% end
%

addpath('./Field2');
savepath = './simulation_data_DiffPSFandScatterer';
if ~exist(savepath, 'dir')
    mkdir(savepath)
end
% --------- fixed parameters ---------
% total # of psf = Npsf*Npt
...
% convert length to # of pixels　=> dt = 1/f0 = ?*1/fs => ? = 1/f0*fs = fs/f0
...
delay_curve = delay_curve * maxdelay(k);         % adjust max phase error referred to k
% ------ Apply delay profile ------
...
newz = interp1(z_pt_start_ind:z_pt_end_ind, beamformed_region_z, linspace(z_pt_start_ind,z_pt_end_ind,Npz));
...
% segment the patch of size (Npz,Npx)
% (Nsz+1)/2: the center index along the specific direction
% (Npz - 1)*specklescale(k)/2: the top N or bottom N indices
