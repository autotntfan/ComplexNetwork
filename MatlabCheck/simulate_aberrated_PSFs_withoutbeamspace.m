...
% for k = 1:4
    ...
    for Nline = 1:beamspace*FOV
        Nline
        f_num_mask_rx = double(z./abs(x_aline(Nline) - x_elem)./2 > f_num); % (Nsample, Nelements)
        f_num_mask_tx = reshape(f_num_mask_rx, Nsample, 1, Nelements); % tx rx mask (Nsample, Nelements, Nelements)
%         delay_rx = double(z./(x_aline(Nline) - x_elem)./2 > f_num).*squeeze(delay(:,Nline,:));
%         delay_tx = reshape(f_num_mask_rx, Nsample, 1, Nelements).*repelem(delay(:,Nline,:), 1,Nelements,1);
%         delay_iline = delay_rx + delay_tx;
%         delay_idx = ceil(Upsample*delay_iline/soundv*fs - Upsample*Noffset);
%         delay_idx(delay_idx > Upsample*Nsample) = Upsample*Nsample + 1;
%         delay_idx(delay_idx <= 0) = Upsample*Nsample + 1;
        delay_iline = delay(:,Nline,:) + squeeze(delay(:,Nline,:)); % tx + rx
        delay_idx = ceil(Upsample*delay_iline/soundv*fs - Upsample*Noffset);
        delay_idx(delay_idx > Upsample*Nsample) = Upsample*Nsample + 1;
        channel_ind = delay_idx + index3D;
