...
function GeneratePSF_diffc(soundv, f0, k)
...
%     end
    ...
    for ipt = 2:size(positions,1)-1 % start from the second to the next to last point location.
        ...
        ylabel('Depth (mm)')
        axis image;
        save(fullfile(savepath,['Data_', num2str(ipt),  '_c_', num2str(soundv), '_delay_', num2str(k), '.mat']), 'psf_rf', 'psf_bb', 'speckle_rf', 'speckle_bb', 'dx', 'dz', 'depth', 'f0', 'k', 'bw', 'delay_curve');
        saveas(fig, fullfile(savepath,['Data_', num2str(ipt),  '_c_', num2str(soundv), '_delay_', num2str(k), '.png']));
