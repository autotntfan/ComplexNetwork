...
import numpy as np
from .fft_test import fft, ifft, fft2, ifft2
from .bn_test import ComplexBN as complex_normalization
from .bn_test import sqrt_init
from .init_test import ComplexInit, ComplexIndependentFilters
...
def conv2d_transpose(
    ...
    '''
    modify conv_utils.deconv_length(height, stride, kernel, padding, output_padding) 
    to conv_utils.deconv_output_length(height, kernel, padding, output_padding,stride)
    '''
...
def conv_transpose_output_length(
    ...
    ''' revise order of function input parameters'''
...
class ComplexConv(Layer):
    ...
    def __init__(
        ...
        self.kernel_shape_complex = None
        self.kernel = None
        self.gamma_rr = None
        self.gamma_ii = None
        self.gamma_ri = None
    ...
    def build(self, input_shape):
        ...
        if False and self.transposed:
            ...
            self.kernel_shape_complex = self.kernel_size + (self.filters*2, input_dim)
        else:
            self.kernel_shape = self.kernel_size + (input_dim, self.filters)
            self.kernel_shape_complex = self.kernel_size + (input_dim, self.filters*2)
    ...
    def call(self, inputs, **kwargs):
        ...
        output.set_shape(self.compute_output_shape(tuple(inputs.shape)))
        return output
