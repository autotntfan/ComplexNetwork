...
def STAbeamform(channeldata,
                ...
                return_type='bb'):
    '''
    Synthetic aperture beamforming with fixed f# for each depth.
    Args:
        channeldata: ndarray, full dataset generated by Field II.
        scat_space: ndarray, scatterer distribution.
        soundv: scalar, speed of sound. [m/s]
        beamformv: scalar, setting speed of sound when beamforming. [m/s]
        pitch: scalar. [m]
        f0: scalar, center freq. [Hz]
        fs: scalar, sampling rate. [Hz]
        Noffset: int, time offset for the first acquistion point unit in sample.
        ptloc: scalar, point location. [m]
        Npx: int, number of pixels along lateral direction.
        Npz: int, number of pixels along axial direction.
        f_num: int, f number.
        beamspacing: int, how many pixel in one beam (beam width).
        gain: scalar, image intensity gain. [dB]
        DR: scalar, dynamic range. [dB]
        psfsegx: scalar, half physical FOVx, x-direction segmentation size. [lambda]
        psfsegz: scalar, half physical FOVz, z-direction segmentation size. [lambda]
        return_type: string, 'bb' or 'rf'.
    Return:
        beamformed baseband/RF psf and speckle.
        
    '''
    return_type = return_type.upper()
    if return_type not in {'RF', 'BB'}:
        raise ValueError(f"Invalid `return_type` of {return_type}, either 'rf', 'RF', 'BB' or 'bb'.")
...
if __name__  == '__main__':
    ...
    psf_bb, speckle_bb = STAbeamform(channeldata,scat_space,soundv,1540,pitch,f0,fs,Noffset,ptloc,257,257) 