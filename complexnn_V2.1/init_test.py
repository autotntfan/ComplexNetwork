...
'''
change:
    from tensorflow.keras import initializers -> from tensorflow.python.ops import init_ops
    initializers._compute_fans -> init_ops._compute_fans
...
class IndependentFilters(Initializer):
    ...
    def __call__(self, shape, dtype=None):
        ...
        if self.nb_filters is not None:
            ...
            fan_in, fan_out = init_ops._compute_fans(
...
class ComplexInit(Initializer):
    ...
    def __init__(self, kernel_size, input_dim,
        ...
        self.criterion = criterion
        self.seed = 1337 if seed is None else seed
        ''' it would be check shape btw in and out        '''
    def __call__(self, shape, dtype=None):
        if self.nb_filters is not None:
