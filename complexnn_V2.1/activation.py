...
class modReLU(Layer):
    ...
    def call(self, inputs):
        ...
        '''
        real = tf.multiply(tf.divide(real,modulus),tf.nn.relu(tf.add(modulus,self.b_factor)))
        imag = tf.multiply(tf.divide(imag,modulus),tf.nn.relu(tf.add(modulus,self.b_factor)))
        return tf.concat([real, imag], axis=-1)
